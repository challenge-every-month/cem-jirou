...
import type { Env, SlackInteractionPayload, ChallengeStatus } from "../../types";
import { AppError } from "../../types";
import { lazyProvision } from "../../services/user";
import { getProjectsWithChallenges, updateProject } from "../../services/project";
...
export function buildReviewModal(
  ...
  for (const ch of challenges) {
    const currentStatus = ch.status === "completed" ? "completed" : "incompleted";
    const initialOption =
      currentStatus === "completed"
        ? { text: { type: "plain_text", text: "✅ 達成" }, value: "completed" }
        : { text: { type: "plain_text", text: "❌ 未達成" }, value: "incompleted" };
...
export async function handleCemReview(
  ...
  const modal = buildReviewModal(publishedProject.id, publishedProject.challenges);
...
export async function handleReviewSubmit(
  ...
  safeWaitUntil(c, (async () => {
    try {
      ...
      const resultLines: Array<{ name: string; status: ChallengeStatus; comment: string | null }> = [];
      ...
      for (const challenge of project.challenges) {
        ...
        const rawStatus = values[statusBlockId]?.[statusActionId]?.selected_option?.value;
        const status: ChallengeStatus =
          rawStatus === "completed" ? "completed" : "incompleted";
        const reviewComment = values[commentBlockId]?.[commentActionId]?.value ?? null;
        ...
        resultLines.push({ name: challenge.name, status, comment: reviewComment });
      }
